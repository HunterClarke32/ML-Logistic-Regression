...
def main():
  ...
  y_pred_test = (X_test_bias @ w >= 0).astype(int)  
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  z = np.dot(X, w)
  logisticVal = logistic(z)
  nll = -np.sum(y * np.log(logisticVal+0.0000001) + (1 - y) * np.log(1 - logisticVal+0.0000001))
  return nll
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
      ...
      z = np.dot(X,w)
      logisticVal = logistic(z)
      w_grad = np.dot(X.T, logisticVal -y)
...
def dummyAugment(X):
  ...
  aug_X = np.hstack((columnOfOnes, X))
  return aug_X
